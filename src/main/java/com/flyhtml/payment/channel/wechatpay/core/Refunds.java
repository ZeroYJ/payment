...
public final class Refunds extends Component {
    ...
    private RefundItem renderRefundItem(Map<String, Object> refundData, int refundItemIndex) {
        ...
        refundItem.setChannel(RefundChannel.from((String) refundData.get(WepayField.REFUND_CHANNEL + "_"
                                                                         + refundItemIndex)));
        refundItem.setRefundFee(Integer.parseInt((String) refundData.get(WepayField.REFUND_FEE + "_"
                                                                         + refundItemIndex)));
        refundItem.setRefundStatus(RefundStatus.from((String) refundData.get(WepayField.REFUND_STATUS + "_"
                                                                             + refundItemIndex)));
        ...
        if (couponRefundCountObj != null) {
            ...
            if (couponRefundCount > 0) {
                ...
                for (int couponItemIndex = 0; couponItemIndex < couponRefundCount; couponItemIndex++) {
                    couponItem = Coupon.newCoupon((String) refundData.get(WepayField.COUPON_REFUND_BATCH_ID + "_"
                                                                          + refundItemIndex + "_" + couponItemIndex),
                                                  (String) refundData.get(WepayField.COUPON_REFUND_ID + "_"
                                                                          + refundItemIndex + "_" + couponItemIndex),
                                                  Integer.parseInt((String) refundData.get(WepayField.COUPON_REFUND_FEE
                                                                                           + "_" + refundItemIndex + "_"
                                                                                           + couponItemIndex)));
