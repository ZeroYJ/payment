...
public final class Refunds extends Component {
  ...
  private RefundItem renderRefundItem(Map<String, Object> refundData, int refundItemIndex) {
    ...
    refundItem.setOutRefundNo(
        (String) refundData.get(WepayField.OUT_REFUND_NO + "_" + refundItemIndex));
    refundItem.setRefundId((String) refundData.get(WepayField.REFUND_ID + "_" + refundItemIndex));
    refundItem.setChannel(
        RefundChannel.from(
            (String) refundData.get(WepayField.REFUND_CHANNEL + "_" + refundItemIndex)));
    refundItem.setRefundFee(
        Integer.parseInt((String) refundData.get(WepayField.REFUND_FEE + "_" + refundItemIndex)));
    refundItem.setRefundStatus(
        RefundStatus.from(
            (String) refundData.get(WepayField.REFUND_STATUS + "_" + refundItemIndex)));

    String settlementRefundFee =
        (String) refundData.get(WepayField.SETTLEMENT_REFUND_FEE + "_" + refundItemIndex);
    if (!Strings.isNullOrEmpty(settlementRefundFee)) {
      refundItem.setSettlementRefundFee(Integer.parseInt(settlementRefundFee));
    }

    refundItem.setRefundRecvAccout(
        (String) refundData.get(WepayField.REFUND_RECV_ACCOUNT + "_" + refundItemIndex));
    ...
    Object couponRefundCountObj =
        refundData.get(WepayField.COUPON_REFUND_COUNT + "_" + refundItemIndex);
        ...
        for (int couponItemIndex = 0; couponItemIndex < couponRefundCount; couponItemIndex++) {
          couponItem =
              Coupon.newCoupon(
                  (String)
                      refundData.get(
                          WepayField.COUPON_REFUND_BATCH_ID
                              + "_"
                              + refundItemIndex
                              + "_"
                              + couponItemIndex),
