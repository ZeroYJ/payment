...
public abstract class Component {
  ...
  protected <T> T doHttpsPost(
      final String url, final Map<String, String> params, Class<T> respClazz) {
    String requestBody = Maps.toXml(params);
    String resp =
        Https.post(url).body(requestBody).ssLSocketFactory(wepay.getSslSocketFactory()).request();
    Map<String, Object> respMap = toMap(resp.replaceAll("(\\r|\\n)", ""));
    doVerifySign(respMap);
  ...
  private XmlReaders readResp(final String xml) {
    ...
    if (WepayField.SUCCESS.equals(returnCode)) {
      ...
      throw new WepayException(
          readers.getNodeStr(WepayField.ERR_CODE), readers.getNodeStr(WepayField.ERR_CODE_DES));
    }
    throw new WepayException(
        readers.getNodeStr(WepayField.RETURN_CODE), readers.getNodeStr(WepayField.RETURN_MSG));
  }
