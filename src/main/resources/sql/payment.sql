...
CREATE TABLE `payment` (
  `paymentId`   VARCHAR(32)   NOT NULL COMMENT '主键',
  `gmtCreate`   DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `gmtModified` DATETIME      NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
  `isPay`       TINYINT       NOT NULL COMMENT '是否已付款',
  `hasRefund`   TINYINT       NOT NULL COMMENT '是否存在退款信息',
  `channel`     VARCHAR(10)   NOT NULL COMMENT '付款渠道',
  `orderNo`     VARCHAR(32)   NOT NULL COMMENT '订单ID',
